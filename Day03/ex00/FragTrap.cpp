...
FragTrap::FragTrap(std::string name) :  _hitPoints(100), _maxHitPoints(100), _energyPoints(100),
									   _maxEnergyPoints(100), _level(1), _name(name), _meleeAttack(30),
										_rangeAttack(20), _armorDamageReduction(5)
{
...
{
	std::cout << "FR4G-TP " << get_name() << " attacks " << target << " at range,";
	std::cout << " causing " << get_rangeAttack() << " points of damage !" << std::endl;
}

void FragTrap::meleeAttack(std::string const &target)
{
	std::cout << "FR4G-TP " << get_name() << " attacks " << target << " at melee,";
	std::cout << " causing " << get_meleeAttack() << " points of damage !" << std::endl;
}

void FragTrap::takeDamage(unsigned int amount)
{
	std::cout << "FR4G-TP " << get_name() << " take " << amount << " damage." << std::endl;
...
{
	std::cout << "FR4G-TP " << get_name() << " be " << amount << " repaired." << std::endl;
	set_hitPoints(get_hitPoints() + amount);
	if (get_hitPoints() > get_maxHitPoints())
		set_hitPoints(get_maxHitPoints());
}
